...
void stack_method_free(stack_t * self);
...
void
stack_method_free(stack_t * self) {
  free(self->points);
}
...
main(void) {
  ...
  int i;
  for (i = 0; i < 3; i++) foo._->push(&foo, ary[i]);
  point_t point = foo._->pop(&foo);
